...
async function init() {
  ...
  for (let i = 0, len = arr.length; i < len; i++) {
    geoObjects[i] = new ymaps.Placemark(arr[i].coordinates, {
      balloonContentHeader: `<a href = "/events/${arr[i]._id}">${arr[i].type}</a> , Дата: ${arr[i].time}`,
      balloonContentBody: `Позвал на тренировку: ${arr[i].author}`,
      balloonContentFooter: `Участвуют: ${arr[i].participants}`,
      hintContent: `Описание: ${arr[i].body}`    
  }, {preset: `${arr[i].icon}`}, {
    iconColor: arr[i].color
  });
}
